...
def load_file(filename):
    try:
        ...
        logger.info('the yaml file load sucessfully')
        return params
    except Exception as e:
        logger.error(f'chek the file path{e}')
        raise
...
def load_data(data_path):
    try:
        ...
        logger.error(f'check the data path {e}')
        raise
...
def save_model(model_path, model):
    try:
        ...
        logger.error(f"model is not saved sucessfully... please check the model path")
        raise
...
def save_data(data_path,train_data, test_data):
    try:
        ...
        logger.error(f"again check the data path {e}")
        raise
...
def main():
    ...
    # load data
    new_df = load_data('./data/interim/new_df.csv')
    ...
    # making directory for saving the model
    os.makedirs("models", exist_ok=True)
    save_model('models/model.pkl',LGRregressor)
    ...
    # making datapath for saving data
